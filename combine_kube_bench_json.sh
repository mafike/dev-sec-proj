...
cat kube-bench-report.json | jq '.Controls' | sed 's/^\[/{ "type": "master", "controls": [/' | sed 's/\]$/] },/' >> combined-bench.json
cat etcd-bench-report.json | jq '.Controls' | sed 's/^\[/{ "type": "etcd", "controls": [/' | sed 's/\]$/] },/' >> combined-bench.json
cat kubelet-bench-report.json | jq '.Controls' | sed 's/^\[/{ "type": "kubelet", "controls": [/' | sed 's/\]$/] }/' >> combined-bench.json
